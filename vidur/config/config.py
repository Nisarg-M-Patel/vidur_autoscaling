...
class MetricsConfig:
    ...
    store_minimal_request_metrics: bool = field(
        default=True,
        metadata={"help": "Whether to store minimal request metrics."},
    )
    ...
    store_autoscaling_metrics: bool = field(
        default=True,
        metadata={"help": "Whether to store autoscaling metrics."},
    )
...
class ClusterConfig:
    num_replicas: int = field(
        default=1,
        metadata={"help": "Number of initial replicas."},
    )
...
@dataclass
class BaseAutoscalerConfig(BasePolyConfig):
...
class SimulationConfig(ABC):
    ...
    autoscaler_config: BaseAutoscalerConfig = field(
        default_factory=InferlineAutoscalerConfig,
        metadata={"help": "Autoscaler config."},
    )
